...
function mockApiResponse(endpoint, options = {}) {
    ...
    if (endpoint.match(/^\/api\/reports\/\d+\/download$/)) {
        // Extract report ID and find matching scan for rich content
        const repId = parseInt(endpoint.split('/')[3]);
        const repEntry = MOCK_REPORTS.find(r => r.id === repId) || MOCK_REPORTS[0] || {};
        const scanId = repEntry.scan_id || 1;
        const scan = MOCK_SCANS.find(s => s.id === scanId) || MOCK_SCANS[0] || {};
        const vulns = MOCK_VULNERABILITIES.filter(v => v.scan_id === scanId);
        ...
        const targetUrl = scan.target_url || 'N/A';

        // Build a rich human-readable PDF using a single page with structured text
        const lines = [
            'BT',
            '/F1 22 Tf 50 760 Td (CyberGuard AI - Security Assessment Report) Tj',
            '/F1 10 Tf 0 -18 Td (─────────────────────────────────────────────────────────────) Tj',
            `/F1 14 Tf 0 -22 Td (SCAN TARGET: ${targetUrl}) Tj`,
            `/F1 10 Tf 0 -16 Td (Report Generated: ${now}) Tj`,
            `/F1 10 Tf 0 -14 Td (Scan ID: ${scanId}   |   Grade: ${scan.grade || 'N/A'}   |   Risk Score: ${scan.risk_score || 'N/A'}/100) Tj`,
            '/F1 10 Tf 0 -18 Td (─────────────────────────────────────────────────────────────) Tj',
            '/F1 13 Tf 0 -22 Td (EXECUTIVE SUMMARY) Tj',
            `/F1 10 Tf 0 -16 Td (Total Vulnerabilities: ${vulns.length}   Critical: ${critCount}   High: ${highCount}   Medium: ${medCount}   Low: ${lowCount}) Tj`,
            `/F1 10 Tf 0 -14 Td (Scan Status: ${scan.status || 'completed'}   Scan Type: ${scan.scan_type || 'full'}) Tj`,
            `/F1 10 Tf 0 -14 Td (Completed: ${scan.completed_at || 'N/A'}) Tj`,
            '/F1 10 Tf 0 -18 Td (─────────────────────────────────────────────────────────────) Tj',
            '/F1 13 Tf 0 -22 Td (VULNERABILITY FINDINGS) Tj',
            ...vulns.slice(0, 8).flatMap((v, i) => [
                `/F1 11 Tf 0 -18 Td (${i+1}. [${(v.severity||'').toUpperCase()}] ${(v.name||v.title||'Finding').substring(0,55)}) Tj`,
                `/F1 9 Tf 0 -13 Td (   CWE: ${v.cwe_id||'N/A'}  CVSS: ${v.cvss_score||'N/A'}  Category: ${v.owasp_category||'N/A'}) Tj`,
                `/F1 9 Tf 0 -12 Td (   Endpoint: ${(v.affected_url||v.affected_endpoint||'N/A').substring(0,60)}) Tj`,
                `/F1 9 Tf 0 -12 Td (   ${(v.description||'').substring(0,80)}) Tj`,
            ]),
            '/F1 10 Tf 0 -20 Td (─────────────────────────────────────────────────────────────) Tj',
            '/F1 13 Tf 0 -22 Td (PORT SCAN RESULTS) Tj',
            '/F1 10 Tf 0 -16 Td (Port 80   TCP   HTTP     nginx/1.18   Open   Risk: Medium) Tj',
            '/F1 10 Tf 0 -14 Td (Port 443  TCP   HTTPS    nginx/1.18   Open   Risk: Low) Tj',
            '/F1 10 Tf 0 -14 Td (Port 22   TCP   SSH      OpenSSH 8.4  Open   Risk: Medium) Tj',
            '/F1 10 Tf 0 -18 Td (─────────────────────────────────────────────────────────────) Tj',
            '/F1 13 Tf 0 -22 Td (SSL/TLS CERTIFICATE ANALYSIS) Tj',
            `/F1 10 Tf 0 -16 Td (Subject: ${targetUrl.replace('https://','').replace('http://','')}   Grade: A) Tj`,
            '/F1 10 Tf 0 -14 Td (Issuer: Let\'s Encrypt   Protocol: TLS 1.3   Key: 2048-bit) Tj',
            '/F1 10 Tf 0 -14 Td (Valid: Yes   Expires: 2027-01-15   Cipher: TLS_AES_256_GCM_SHA384) Tj',
            '/F1 10 Tf 0 -18 Td (─────────────────────────────────────────────────────────────) Tj',
            '/F1 13 Tf 0 -22 Td (SECURITY HEADERS ANALYSIS) Tj',
            '/F1 10 Tf 0 -16 Td (Content-Security-Policy        FAIL  Not Set) Tj',
            '/F1 10 Tf 0 -14 Td (Strict-Transport-Security       PASS  max-age=31536000) Tj',
            '/F1 10 Tf 0 -14 Td (X-Frame-Options                FAIL  Not Set) Tj',
            '/F1 10 Tf 0 -14 Td (X-Content-Type-Options          PASS  nosniff) Tj',
            '/F1 10 Tf 0 -14 Td (Referrer-Policy                WARN  unsafe-url) Tj',
            '/F1 10 Tf 0 -18 Td (─────────────────────────────────────────────────────────────) Tj',
            '/F1 13 Tf 0 -22 Td (DIRECTORY & PATH DISCOVERY) Tj',
            '/F1 10 Tf 0 -16 Td (/admin         200  High    Sensitive path - review access control) Tj',
            '/F1 10 Tf 0 -14 Td (/backup        403  Medium  Restricted but discoverable) Tj',
            '/F1 10 Tf 0 -14 Td (/api/v1        200  Low     Public API endpoint) Tj',
            '/F1 10 Tf 0 -14 Td (/.env          404  High    Potential config file exposure) Tj',
            '/F1 10 Tf 0 -18 Td (─────────────────────────────────────────────────────────────) Tj',
            '/F1 9 Tf 0 -18 Td (CONFIDENTIAL - Generated by CyberGuard AI Threat Detection Platform) Tj',
            'ET'
        ];
        ...
        const pdfParts = [
            ...
            `3 0 obj<</Type/Page/MediaBox[0 0 612 900]/Parent 2 0 R/Resources<</Font<</F1 4 0 R>>>>/Contents 5 0 R>>endobj\n`,
